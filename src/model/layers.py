...
class Residual(nn.Module):
    def __init__(
        self, channels_in, filters, kernel_size=5, dilation_rate=1, dropout_rate=0.1
    ):
...
class Attention(nn.Module):
    ...
    def forward(self, x, use_dist=False):
        ...
        if use_dist:
            sim = -torch.linalg.vector_norm(
                q.unsqueeze(-1) - k.unsqueeze(-2), ord=2, dim=-3
            )
        else:
...
class AddNoise(nn.Module):
    ...
    def forward(self, inputs):
        rand = torch.randn(
            [inputs.shape[0], inputs.shape[1], 1, 1], dtype=torch.float32
        )
...
class ResBlock(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Conv2d(
            inp, filters, kernel_size=kernel_size, stride=(1, 1), padding=padding
        )
        ...
        if upsample:
            ...
            self.res_conv = nn.Conv2d(
                inp // 2, filters, kernel_size=(1, 1), stride=(1, 1), padding=0
            )
        else:
            ...
            self.res_conv = nn.Conv2d(
                inp, filters, kernel_size=(1, 1), stride=(1, 1), padding=0
            )
        if self.emb:
    ...
    def forward(self, x):
        ...
        if self.upsample:
            y = pixel_shuffle(y)
        elif self.strides != (1, 1) and (
            self.padding == "same" or self.padding != (0, 0)
        ):
            y = F.avg_pool2d(
                y,
                self.strides,
                padding=(y.shape[-2] % self.strides[0], y.shape[-1] % self.strides[1]),
            )
